...
<xsl:variable name="default.specdoc">
  <xsl:choose>
    ...
    <xsl:when test="contains(/spec/header/title, 'Data Model')">
      <xsl:value-of select="'DM'"/>
      <xsl:message>specdoc is DM</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'Functions and Operators')">
      <xsl:value-of select="'FO'"/>
      <xsl:message>specdoc is FO</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'XML Path Language')">
      <xsl:value-of select="'XP'"/>
      <xsl:message>specdoc is XP</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'An XML Query Language')">
      <xsl:value-of select="'XQ'"/>
      <xsl:message>specdoc is XQ</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'Formal Semantics')">
      <xsl:value-of select="'FS'"/>
      <xsl:message>specdoc is FS</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'XQueryX')">
      <xsl:value-of select="'XQX'"/>
      <xsl:message>specdoc is XQX</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'Serialization')">
      <xsl:value-of select="'SER'"/>
      <xsl:message>specdoc is SER</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'XML Query Use Cases')">
      <xsl:value-of select="'XQUC'"/>
      <xsl:message>specdoc is XQUC</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'Full-Text Use Cases')">
      <xsl:value-of select="'FTUC'"/>
      <xsl:message>specdoc is FTUC</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'Full-Text')">
      <xsl:value-of select="'FT'"/>
      <xsl:message>specdoc is FT</xsl:message>
    </xsl:when>
    <xsl:when test="contains(/spec/header/title, 'XSLT')">
    ...
    <xsl:otherwise>
      ...
      <xsl:message terminate="yes">
        <xsl:text>Failed to determine which specdoc this is: use specdoc parameter!</xsl:text>
      </xsl:message>
    </xsl:otherwise>
  </xsl:choose>
</xsl:variable>
...
<xsl:template match="bibl" name="bibl">
  <xsl:variable name="xsl-query" select="document('../etc/xsl-query.xml')"/>
