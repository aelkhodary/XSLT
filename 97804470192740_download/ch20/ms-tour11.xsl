<?xml-stylesheet type="text/xsl" href="ms-tour.xsl"?>
<xsl:transform
...
<!--
    ...
    ** This version is adapted to MSXML3 by using the msxsl:node-set() extension function.
...
<xsl:template name="place-knight">
    ...
    <xsl:copy-of select="msxsl:node-set($board)/square[position()-1 &lt; $square]"/>
...
<xsl:template name="make-moves">
    ...
    <xsl:call-template name="try-possible-moves">
        ...
        <xsl:with-param name="possible-moves" 
                        select="msxsl:node-set($possible-move-list)/move"/>
    </xsl:call-template>
...
<xsl:template name="try-possible-moves">
    ...
    <xsl:when test="$possible-moves">
        ...
        <xsl:variable name="final-board">
            <xsl:choose>
            <xsl:when test="msxsl:node-set($next-board)/square = ''">   
...
<xsl:template name="find-best-move">
    ...
    <xsl:variable name="number-of-exits" 
                  select="count(msxsl:node-set($trial-move-exit-list)/move)"/>
...
<xsl:template name="list-possible-moves">
    ...
    <xsl:variable name="bd" select="msxsl:node-set($board)"/>

    <xsl:if test="$row &gt; 1 and $column &gt; 0 and $bd/square[($square - 17) + 1]=''">
        <move><xsl:value-of select="$square - 17"/></move>
    </xsl:if>
    <xsl:if test="$row &gt; 1 and $column &lt; 7 and $bd/square[($square - 15) + 1]=''">
        <move><xsl:value-of select="$square - 15"/></move>
    </xsl:if>
    <xsl:if test="$row &gt; 0 and $column &gt; 1 and $bd/square[($square - 10) + 1]=''">
        <move><xsl:value-of select="$square - 10"/></move>
    </xsl:if>
    <xsl:if test="$row &gt; 0 and $column &lt; 6 and $bd/square[($square - 6) + 1]=''">
        <move><xsl:value-of select="$square - 6"/></move>
    </xsl:if>
    <xsl:if test="$row &lt; 6 and $column &gt; 0 and $bd/square[($square + 15) + 1]=''">
        <move><xsl:value-of select="$square + 15"/></move>
    </xsl:if>
    <xsl:if test="$row &lt; 6 and $column &lt; 7 and $bd/square[($square + 17) + 1]=''">
        <move><xsl:value-of select="$square + 17"/></move>
    </xsl:if>
    <xsl:if test="$row &lt; 7 and $column &gt; 1 and $bd/square[($square + 6) + 1]=''">
        <move><xsl:value-of select="$square + 6"/></move>
    </xsl:if>
    <xsl:if test="$row &lt; 7 and $column &lt; 6 and $bd/square[($square + 10) + 1]=''">
        <move><xsl:value-of select="$square + 10"/></move>
    </xsl:if>
</xsl:template>
