...
<!--
    ...
    Minor revisions: September 2007
    
    ** This version of the stylesheet uses XSLT 2.0 and XPath 2.0, with sequences and functions.
    ** Type declarations added in Nov 2002 version.

    This stylesheet can be run by nominating "main" as the initial template. 
    Alternatively, it can be run by using any XML file as a dummy source document.
...
<xsl:function name="tour:find-best-move" as="xs:integer*" >
    ...
    <xsl:sequence
        select="if (exists($other-possible-moves))
